 ...
 */
public class TableFixColHeaders extends ViewGroup {
	
	private static final String TAG = "TableFixColHeaders";
	...
	private int fixColWidth;
	private boolean horizionalScroll = false;
	 ...
	 */
	public TableFixColHeaders(Context context) {
		this(context, null);
	}
	 ...
	 */
	public TableFixColHeaders(Context context, AttributeSet attrs) {
		super(context, attrs);

		this.headView = new ArrayList<View>();
		this.rowViewList = new ArrayList<View>();
		this.columnViewList = new ArrayList<List<View>>();
	...
	private void setFixCol(int index){
		if(index < 0){
			this.fixCol = -1;
		}else if(adapter != null && index >= adapter.getColumnCount()){
			this.fixCol = adapter.getColumnCount() - 1;
		}else{
			this.fixCol = index;
		}
	}
	...
	public void setAdapter(TableAdapter adapter,int fixCol){
		setAdapter(adapter);
		setFixCol(fixCol);
	}
	...
	public void scrollBy(int x, int y) {
		...
		if(horizionalScroll){
			if (scrollX == 0) {
				...
				while (widths[firstColumn + 1 + fixCol] < scrollX) {
					if (!rowViewList.isEmpty()) {
						removeLeft();
					}
					scrollX -= widths[firstColumn + 1 + fixCol];
					firstColumn++;
				}
				while (getFilledWidth() < width) {//如果需要显示新的一列
					addRight();
				}
			} else {
				while (!rowViewList.isEmpty() && getFilledWidth() - widths[firstColumn + fixCol + rowViewList.size()] >= width) {
					removeRight();
				}
				if (rowViewList.isEmpty()) {
					while (scrollX < 0) {
	...
	public int getActualScrollX() {
		return scrollX + sumArray(widths, 1 + fixCol, firstColumn);
	}
	...
	private int getFilledWidth() {
		return fixColWidth + sumArray(widths, firstColumn + fixCol + 1, rowViewList.size()) - scrollX;
	}
	...
	private void addLeft() {
		addLeftOrRight(firstColumn + fixCol, 0);
	}
	...
	private void addRight() {
		...
		addLeftOrRight(firstColumn + size + fixCol + 1 , size);
	}
	...
	private void addLeftOrRight(int column, int index) {
		View view = makeView(-1, column, widths[column], heights[0]);
		rowViewList.add(index, view);

		int i = firstRow;
		for (List<View> list : bodyViewTable) {
			view = makeView(i, column, widths[column], heights[i + 1]);
			list.add(index, view);
			i++;
		}
	}
	...
	private void addTopAndBottom(int row, int index) {
		...
		for (int i = 0; i <= fixCol; i++) {
			view = makeView(row, i, widths[i],heights[row +1]);
			list.add(view);
		}
		columnViewList.add(list);
		
		list = new ArrayList<View>();
		final int size = rowViewList.size() + firstColumn;
		for (int i = firstColumn; i < size; i++) {
			view = makeView(row, i, widths[i + 1 + fixCol], heights[row + 1]);
			list.add(view);
		}
	...
	private void removeTopOrBottom(int position) {
		List<View> colHeaders = columnViewList.remove(position);
		for (View view : colHeaders) {
			removeView(view);
		}
	...
	private void repositionViews() {
		...
		left = fixColWidth - scrollX;
		i = firstColumn + fixCol + 1;
		...
		i = firstRow;
		for (List<View> list : columnViewList) {
			bottom = top + heights[++i];
			left = 0;
			for (int j = 0; j <= fixCol; j++) {
				right = left + widths[j];
				list.get(j).layout(left, top, right, bottom);
				left = right;
			}
			top = bottom;
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (adapter != null) {
			...
			int sumWidths = sumArray(widths);
			if (widthMode == MeasureSpec.AT_MOST) {
				w = Math.min(widthSize, sumWidths);
			} else if (widthMode == MeasureSpec.UNSPECIFIED) {
				w = sumWidths;
			} else {
				w = widthSize;
				int sumArray = sumWidths;
			...
			fixColWidth = sumArray(widths, 0, fixCol + 1);
			horizionalScroll = true;
			if(w <= fixColWidth || w >= sumWidths){
				horizionalScroll = false;
			}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (needRelayout || changed) {
			...
			if (adapter != null) {
				...
				if(horizionalScroll){
					addShadow(shadows[0], fixColWidth, 0, fixColWidth + shadowSize, bottom);//left
					addShadow(shadows[2], right - shadowSize, 0, right, bottom);//right
				}
				addShadow(shadows[1], 0, heights[0], right, heights[0] + shadowSize);//top
				addShadow(shadows[3], 0, bottom - shadowSize, right, bottom);//buttom
				...
				left = fixColWidth - scrollX;
				for (int i = fixCol + 1; i < columnCount && left < width; i++) {
					right = left + widths[i];
					final View view = makeAndSetup(-1, i, left, 0, right, heights[0]);
				...
				for (int i = firstRow; i < rowCount && top < height; i++) {
					...
					left = fixColWidth - scrollX;
					List<View> list = new ArrayList<View>();
					for (int j = firstColumn + fixCol + 1; j < columnCount && left < width; j++) {
						right = left + widths[j];
						final View view = makeAndSetup(i, j, left, top, right, bottom);
						list.add(view);
						left = right;
	...
	private int scrollXBounds(int desiredScroll, int firstCell, int sizes[], int viewSize) {
		if (desiredScroll == 0) {
			...
			desiredScroll =  Math.max(desiredScroll, -sumArray(sizes, fixCol + 1, firstCell));
		} else {
			desiredScroll = Math.min(desiredScroll, Math.max(0, sumArray(sizes, firstCell + fixCol + 1, sizes.length - fixCol - 1 - firstCell) + sumArray(sizes,0,fixCol + 1) - viewSize));
		}
	...
	private int[] adjustXFirstCellsAndScroll(int scroll, int firstCell, int sizes[]) {
		if (scroll == 0) {
			...
			while (sizes[firstCell + fixCol + 1] < scroll) {
				firstCell++;
	...
	protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
		...
		if (row == null || (row == -1 && column <= fixCol)) {
			ret = super.drawChild(canvas, child, drawingTime);
		} else {
			canvas.save();
			if (row == -1) {//行头
				canvas.clipRect(fixColWidth, 0, canvas.getWidth(), canvas.getHeight());
			} else if (column <= fixCol) {//列头
				canvas.clipRect(0, heights[0], canvas.getWidth(), canvas.getHeight());
			} else {
				canvas.clipRect(fixColWidth, heights[0], canvas.getWidth(), canvas.getHeight());
			}
			ret = super.drawChild(canvas, child, drawingTime);
			canvas.restore();
		}
	...
	private View makeView(int row, int column, int w, int h) {
		...
		if (itemViewType == TableAdapter.IGNORE_ITEM_VIEW_TYPE) {//忽略View的重用
			recycledView = null;
		} else {
